...
export default class Sonos {
  ...
  async process(cmd, { delay } = {}) {
    ...
    if (serviceCommand) {
      ...
      const shuffle = await this.#requestRoom('state').then((r) => !r.playMode.shuffle)
      const needsShuffle = parts.includes('playlist')
      log.verbose('Shuffle', { shuffle, needsShuffle })
      if (shuffle !== needsShuffle) {
        await this.#requestRoom(`shuffle/${needsShuffle ? 'on' : 'off'}`, this.#delay)
      }
