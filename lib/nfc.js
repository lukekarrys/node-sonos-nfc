...
const processNfcCard = async (sonos, reader) => {
  try {
  ...
  const isFormatedAsNDEF = nfcCard.isFormatedAsNDEF()
  const hasReadPermissions = nfcCard.hasReadPermissions()
  const hasNDEFMessage = nfcCard.hasNDEFMessage()
  ...
  if (isFormatedAsNDEF && hasReadPermissions && hasNDEFMessage) {
    try {
      ...
      for (const record of NDEFMessage) {
        log.silly(record)
        try {
          const res = await sonos.process(
            record.uri ??
              record.text ??
              record.NDEFlibRecord.getPayload()
  ...
  new Promise((resolve, reject) => {
    ...
    nfc.on('reader', (reader) => {
      ...
      reader.on('card', async (card) => {
        ...
        log.silly(card)
        await processNfcCard(sonos, reader)
      })

      reader.on('card.off', (card) =>
        log.silly(`Card removed: ${card.type} - ${card.uid}`)
      )
